...
exports.run = (client, message, args) => {
  ...
  if (message.guild.member(user).roles.has(role.id)) {
    message.channel.sendEmbed(new Discord.RichEmbed()
      .setColor(0x00EB1A1A)
      .setTimestamp()
      .addField(`Error :no_entry:`, `Deze gebruiker heeft al de role **${role2}**!`)).then(m => m.delete(10000));
  } else {
    message.guild.member(user).addRole(role).then(() => {
      message.channel.sendEmbed(new Discord.RichEmbed()
        .setColor(0x00AB29D4)
        .setTimestamp()
        .addField(`Succesvol :white_check_mark:`, `**${user}** heeft nu de role **${role2}**.`)).then(m => m.delete(10000));
    }); 
  }
};
...
exports.help = {
  name: 'giverole',
  rank: 'Owner',
  description: '(OWNER) - Geeft opgegeven role aan opgegeven persoon',
